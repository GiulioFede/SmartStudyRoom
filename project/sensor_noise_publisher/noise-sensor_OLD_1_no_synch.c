...
//manca la funzione per la gestione di publishing
		...
		if((ev == PROCESS_EVENT_TIMER && etimer_expired(&periodic_timer)) || ev == PROCESS_EVENT_POLL){
			...
			if(state == STATE_CONNECTED){
				...
				if(ev == PROCESS_EVENT_TIMER && etimer_expired(&publish_timer)){
					...
					sprintf(app_buffer, "{\"timestamp\": %ld,\"idBuilding\": %d, \"idRoom\": %d, \"idTable\": %d, \"value\": %s}", time(NULL), ID_BUILDING, ID_ROOM, node_id, convert_float_to_string(value));
